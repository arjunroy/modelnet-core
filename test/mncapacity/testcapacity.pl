...
{
	`rm -f r1.dat`;	
	system("ssh xzong\@$svrname 'sudo netstat -sw | grep forwarded >> /home/xzong/mnproject/mncapacity/r1.dat'");
	$numhops=1;
	while ($numhops <= 12)
	{	
...
#`xmlformat.pl capacity.model > capacity1.model`;
		...
		`gexec deployhost /home/xzong/mnproject/mncapacity/capacity.model /home/xzong/mnproject/mncapacity/capacity.route` or 
		die("Error: gexec error!!\n"); 	#redeploy modelnet on sysnet27
			for ($i=0; $i<$numflows; $i++)  {
				my $tmp=$i*($numhops+1)+$numhops;
				$pid=Parallel::Jobs::start_job("gexec vnrunhost $tmp /home/xzong/mnproject/mncapacity/capacity.model netserver -p $port") or 
				die("Error: gexec error!!\n");
				$port++;
			}
		...
		for ($i=0; $i<$numflows; $i++)  {
			...
			$tmp=$tmp-$numhops;
			print $tmp."       ".$addr."\n";
			$pid=Parallel::Jobs::start_job("gexec vnrunhost $tmp /home/xzong/mnproject/mncapacity/capacity.model netperf -H $addr -p $port -l 20 -- -m 1470 -s 128k -S 128k") or die ("Error: gexec error!!\n");
			$port++;
		}
		sleep 25;
