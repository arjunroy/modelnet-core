...
double    EIGEN_TOLERANCE = 1e-4;	/* Numerical eigen-tolerance */
double    SRESTOL = -1.;		/* Rel resid tol on T evec; autoset if <= 0 */
int       LANCZOS_SO_INTERVAL = 5;	/* Itns. between SO orthog checks; set >= 2 */
...
int       KL_NTRIES_BAD = 5;	/* # unhelpful passes before quitting KL */
...
double    COARSEN_RATIO_MIN = 0.9;  /* Min vtx reduction each coarsen stage */
int       COARSE_NLEVEL_KL = 1;	   /* # levels between KL calls in uncoarsening */
int       MATCH_TYPE = 5;	   /* Type of contraction matching (1..5) */
int       HEAVY_MATCH = TRUE;	   /* Encourage heavy match edges? (TRUE/FALSE) */
...
int       REFINE_PARTITION = TRUE;	/* Postprocess to improve cuts? */
...
int       ARCHITECTURE = 1;	/* 0=> hypercube, d=> d-dimensional mesh (0..3)*/
